...
class MovilizadorController extends Controller
{
  ...
  public function index() {
    if(!Auth::user() || Auth::user()->level != "Movilizador")
  ...
  public function createNewFamiliar( $parent_id = 0, $isCreate = 1 ) {

    $weight = config('dz.public.weight');

    if ( ! Auth::user() || $weight[Auth::user()->level] > $weight['Movilizador'] ) {
        return redirect( '/dashboard' );
    }
    if ( $parent_id === 0 ) {
    ...
    return view( 'movilizador.createNewFamiliar', compact( 'page_title', 'page_description', 'action', 'states', 'auto_password', 'parent_id', 'level', 'u_user' ) );
}

public function editFamiliar($user_id) {
    return $this->createNewFamiliar($user_id, 0);
  ...
  public function create(Request $request) {
    $weight = config('dz.public.weight');
		...
		if ( ! Auth::user() || ! $request->input( 'level' ) || ! ( $weight[ Auth::user()->level ] < $weight[ $request->input( 'level' ) ] ) ) {
            ...
            if($validated['edad_result'] == '16' || $validated['edad'] == '17') $validated['curp'] = $validated['edad'];
            else $validated['elector_key'] = $validated['edad'];
  ...
  public function edit( $user_id, Request $request ) {
    $weight = config('dz.public.weight');
		...
		if ( ! Auth::user() || ! $request->input( 'level' ) || ! ( $weight[ Auth::user()->level ] < $weight[ $request->input( 'level' ) ] ) ) {
            ...
            if($validated['edad_result'] == '16' || $validated['edad'] == '17')
            {
                $validated['elector_key'] = null;
                $validated['curp'] = $validated['edad'];
            }
            else {
                $validated['elector_key'] = $validated['edad'];
                $validated['curp'] = null;
            }
